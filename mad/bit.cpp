 ...
 * DESCRIPTION:	initialize bit pointer struct
 */
TMadBit::TMadBit(unsigned char const *lbyte)
{
  byte  = lbyte;
  cache = 0;
  left  = CHAR_BIT;
 ...
 * DESCRIPTION:	initialize bit pointer struct
 */
void TMadBit::SetBuffer(unsigned char const *lbyte)
{
  byte  = lbyte;
  cache = 0;
  left  = CHAR_BIT;
 ...
 */
unsigned int TMadBit::GetDistance(TMadBit const *begin,
			    TMadBit const *end)
{
 ...
 */
unsigned char const *TMadBit::GetNextByte()
{
  if (byte)
    return left == CHAR_BIT ? byte : byte + 1;
  else
    return 0;
}
 ...
 */
void TMadBit::Skip(unsigned int len)
{
  if (byte)
  {
    byte += len / CHAR_BIT;
 ...
 */
unsigned long TMadBit::Read(unsigned int len)
{
 ...
 */
unsigned short TMadBit::CalcCrc(unsigned int len,
			   unsigned short init)
{
  ...
  if (byte)
  {
    ...
    {
      case 3:
        crc = (crc << 8) ^ crc_table[((crc >> 8) ^ Read(8)) & 0xff];
